...
class CourseSearchTool(Tool):
    ...
    def _format_results(self, results: SearchResults) -> str:
        ...
        for doc, meta in zip(results.documents, results.metadata):
            ...
            # Track source for the UI, with a link to the lesson (or course) video/page
...
class CourseOutlineTool(Tool):
    ...
    def execute(self, course_name: str) -> str:
        ...
        lessons = sorted(
            course.get("lessons", []), key=lambda lesson: lesson.get("lesson_number", 0)
        )
        ...
        self.last_sources = [
            {"text": course["title"], "link": course.get("course_link")}
        ]
        return "\n".join(lines)
